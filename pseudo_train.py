...
pseudo_train_dataset = TensorDataset(torch.from_numpy(NAN_inputs).to(DEVICE))

print("Define dataloaders")
train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)
val_loader = DataLoader(val_dataset, batch_size=64, shuffle=False)
pseudo_loader = DataLoader(pseudo_train_dataset, batch_size=64, shuffle=False)
...
print("begin pseudo-training loop")
for epoch in range(1):
    
    # disable gradient computation for validation
    with torch.no_grad():
        # loop over the validation data loader
        for i, data in enumerate(pseudo_train_loader, 0):
            # get the inputs and labels from the data loader
            inputs = data
